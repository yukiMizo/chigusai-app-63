...
import '../../../data/zentai_schedule.dart';

class BunkasaiPage1 extends StatelessWidget {
  const BunkasaiPage1({super.key});

  final List<Tab> _tabs = const [Tab(text: "全体", height: 30), Tab(text: "演劇", height: 30), Tab(text: "有志", height: 30)];

  Widget _buildZentaiCard(BuildContext context, ZentaiDetailData zentaiDetailData) {
    return Card(
      child: ListTile(onTap: () {}, leading: Text(zentaiDetailData.startTime.getTimeAsString()), title: Text(zentaiDetailData.title)),
    );
  }
  ...
  Widget _buildEngekiCard(BuildContext context, EngekiDetailData engekiDetailData) {
    return Card(
      child: ListTile(
        onTap: () {},
        leading: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(engekiDetailData.startTime.getTimeAsString()), Text(engekiDetailData.endTime.getTimeAsString())]),
  ...
  Widget _buildYushiCard(BuildContext context, YushiDetailData yushiDetailData) {
    return Card(
      child: ListTile(onTap: () {}, leading: Text(yushiDetailData.startTime.getTimeAsString()), title: Text(yushiDetailData.title)),
    );
  }
        ...
        SizedBox(
          ...
          child: Padding(
            ...
            child: const Column(
              ...
              children: [Text("文化祭１日目", style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold))],
            ),
          ),
        ),
      ]),
    ...
    return DefaultTabController(
      ...
      child: NestedScrollView(
        headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
          return <Widget>[_buildHeaderSection(), _buildTabSection()];
        },
        body: TabBarView(
