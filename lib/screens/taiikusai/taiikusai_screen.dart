...
class _TaiikusaiScreenState extends ConsumerState<TaiikusaiScreen> {
    ...
    return Padding(
      ...
      child: Card(
        ...
        child: ListTile(
          ...
          title: Text(taiikusaiDetailData.title, style: const TextStyle(fontSize: 20)),
  ...
  Widget _buildHeader() {
    return const SizedBox(
      ...
      child: Padding(
        ...
        child: Text('体育祭', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Future _loadData() async {
    ...
    try {
      _taiikusaiDataList.sort(
        ((a, b) => (_loadedTimeMap[a.title]!["hour"] * 100 + _loadedTimeMap[a.title]!["minute"]).compareTo((_loadedTimeMap[b.title]!["hour"] * 100 + _loadedTimeMap[b.title]!["minute"]))),
      );
    } catch (_) {}
    setState(() {});
  }
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  IconButton(
                    onPressed: () async {
                      DateTime? selectedTime = await picker.DatePicker.showTimePicker(context, showTitleActions: true, showSecondsColumn: false, currentTime: newTime, locale: picker.LocaleType.jp);
                    ...
                    child: FilledButton(
                      onPressed: newTime == currentTime
                          ...
                          : () async {
                              ...
                              try {
                                _taiikusaiDataList.sort(
                                  ((a, b) => (_loadedTimeMap[a.title]!["hour"] * 100 + _loadedTimeMap[a.title]!["minute"]).compareTo(
                                    (_loadedTimeMap[b.title]!["hour"] * 100 + _loadedTimeMap[b.title]!["minute"]),
                                  )),
                                );
                              } catch (_) {}
                              if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          //_loadData();
          /*await FirebaseFirestore.instance.collection("taiikusaiResult").doc("taiikusaiResultDoc").set({
            "大玉転がし": [],
            "綱引き": [],
            "ブロリレ予選": [],
            "マジクラ": [],
          ...
          /*await FirebaseFirestore.instance.collection("taiikusaiTime").doc("taiikusaiTimeDoc").set({
            "開場": {"hour": 9, "minute": 0},
            "ST": {"hour": 9, "minute": 45},
            "開会式": {"hour": 10, "minute": 00},
            "大玉転がし": {"hour": 10, "minute": 30},
            "綱引き": {"hour": 11, "minute": 00},
            "ブロリレ予選": {"hour": 11, "minute": 40},
            "昼休憩": {"hour": 12, "minute": 0},
            "マジクラ": {"hour": 12, "minute": 50},
            "玉入れ": {"hour": 13, "minute": 30},
            "ブロパフォ": {"hour": 13, "minute": 35},
            "ブロリレ決勝": {"hour": 14, "minute": 35},
            "閉会式": {"hour": 14, "minute": 45},
            "完全退場": {"hour": 15, "minute": 30},
          }, SetOptions(merge: true));*/
        },
        child: Scrollbar(
      ...
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.of(context).pushNamed(ResultScreen.routeName), child: const Icon(Icons.scoreboard)),
    );
  }
}
