...
class _UpdateResultScreenState extends State<UpdateResultScreen> {
                ...
                onPressed: (_placeController.text == "" || _classNameController.text == "" || _pointController.text == "")
                    ...
                    : () {
                        if (_awardBlockDataMap[title] == null) {
                          _awardBlockDataMap[title] = AwardBlockData(
                            title: title,
                            rankWidgetDataList: [RankWidgetData(place: int.parse(_placeController.text), className: _classNameController.text, point: int.parse(_pointController.text))],
                          );
                        } else {
                          _awardBlockDataMap[title]!.rankWidgetDataList.add(
                            RankWidgetData(place: int.parse(_placeController.text), className: _classNameController.text, point: int.parse(_pointController.text)),
                          );
                        }
                        setState(() {
                          _isDirty = true;
    ...
    await FirebaseFirestore.instance.collection("taiikusaiResult").doc("taiikusaiResultDoc").get().then((DocumentSnapshot doc) {
      ...
      data.forEach((title, resultList) {
        ...
        for (var map in resultList) {
          rankWidgetDataList.add(RankWidgetData(place: map["place"], point: map["point"], className: map["className"]));
        }
  ...
  Future _updateFirestoreData() async {
    ...
    _awardBlockDataMap.forEach((title, awardBlockData) {
      ...
      for (var data in awardBlockData.rankWidgetDataList) {
        listToAdd.add({"place": data.place, "className": data.className, "point": data.point});
      }
  ...
  Widget build(BuildContext context) {
    ...
    taiikusaiDataList.add(
      const TaiikusaiDetailData(
        title: "全体",
        startTime: Time(day: GakusaiDay.taiikusai, hour: 20),
        showResult: true,
      ),
    );
    if (_isInit) {
              ...
              child: SingleChildScrollView(
                child: Padding(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Padding(padding: const EdgeInsets.only(top: 20), child: _buildEditAwardBlock(taiikusaiDataList[index].title));
                    },
                  ),
                ),
              ),
