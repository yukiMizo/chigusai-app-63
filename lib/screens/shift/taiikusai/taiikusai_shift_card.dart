...
class TaiikusaiShiftCard extends StatefulWidget {
  ...
  const TaiikusaiShiftCard({super.key, required this.loadedTimeMap, required this.day, required this.taiikusaiDataList, required this.index});
...
class _TaiikusaiShiftCardState extends State<TaiikusaiShiftCard> {
  ...
  Future<void> _setLocalNotification() async {
    ...
    await NotificationManager.registerLocNotification(id: _getId, day: widget.day, hour: hour, minute: minute, title: "シフト", message: message);
  }
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(duration: Duration(milliseconds: 1500), content: Text("通知オン　1種目前に通知します")));
                ...
                SizedBox(
                  ...
                  child: OutlinedButton(
                    ...
                    onPressed: () {
                      ...
                      _setLocalNotification();
                      ScaffoldMessenger.of(context).hideCurrentSnackBar();
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(duration: Duration(milliseconds: 1500), content: Text("通知オン　2種目前に通知します")));
                      Navigator.pop(context);
                    },
                    child: const Text("出場"),
                  ),
      ...
      child: TextButton(
        onPressed: () {
          if (_notifyType == NotifyType.noNotify) {
            ...
            ScaffoldMessenger.of(context).hideCurrentSnackBar();
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(duration: Duration(milliseconds: 1500), content: Text("通知オフ")));
          }
        },
        child: Column(
          children: [
            const SizedBox(height: 5),
            Icon(Icons.notifications, size: 28, color: _notifyType != NotifyType.noNotify ? Theme.of(context).colorScheme.secondary : Colors.grey),
            Text(
              switch (_notifyType) {
              ...
              style: TextStyle(fontSize: 15, color: _notifyType != NotifyType.noNotify ? Theme.of(context).colorScheme.secondary : Colors.grey),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: ListTile(
        ...
        title: Text(widget.taiikusaiDataList[widget.index].title, style: const TextStyle(fontSize: 20)),
