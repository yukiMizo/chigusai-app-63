...
class BunkasaiShiftData {
  ...
  BunkasaiShiftData({required this.id, required this.title, required this.startTime, required this.endTime, required this.color, required this.notificationTime});

  void updateData({String? title, Time? startTime, Time? endTime, Time? notificationTime, Color? color}) {
    if (title != null) {
      this.title = title;
    }
    if (startTime != null) {
...
class _BunkasaiShiftScreenState extends State<BunkasaiShiftScreen> {
  ...
  final List<Color> _colors = [const Color(0xff2196f3), const Color(0xff4caf50), const Color(0xfff44336), const Color(0xffff9800), const Color(0xffffeb3b)];
    ...
    return Card(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: Border(left: BorderSide(color: bunkasaiShiftData.color, width: 5)),
        ),
        child: ListTile(
          onTap: () => showDialog(
            context: context,
          ...
          leading: Text(bunkasaiShiftData.startTime.getTimeAsString(), style: const TextStyle(fontSize: 20)),
          title: Text(bunkasaiShiftData.title, style: const TextStyle(fontSize: 20)),
        ),
      ),
    );
  }
          ...
          actions: <Widget>[
            if (!dialogIsLoading)
              SizedBox(
                width: 140,
                height: 40,
                child: TextButton(onPressed: () => Navigator.pop(context), child: const Text("キャンセル")),
              ),
            if (!dialogIsLoading)
              SizedBox(
                width: 140,
                ...
                child: FilledButton(
                  onPressed: _titleController.text == ""
                      ...
                      : () async {
                          ...
                          final List<String> bunkasaiShiftList = await LocalData.readLocalData<List>(dataName) ?? [];
